...
def upgrade() -> None:
    ...
    sa.Column('source', sa.Enum('SPREADSHEET', 'LANDING_PAGE', 'DIRF', name='clientsource'), nullable=False),
    sa.Column('mother_name', sa.String(), nullable=True),
    sa.Column('document_type', sa.Enum('RG', 'CNH', 'PEP', name='clientdocumenttype'), nullable=True),
    ...
    sa.Column('search_irpf_status', sa.Enum('NOT_SEARCHED', 'SEARCHING', 'SUCCESS', 'ERROR', name='clientsearchirpfstatus'), nullable=True),
    ...
    sa.Column('registration_status', sa.Enum('PENDING_DOCUMENTS', 'FINISHED', name='clientregistrationstatus'), nullable=True),
    ...
    op.create_table('motor_runnings',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('status', sa.Enum('in_progress', 'paused', 'finished', name='motorrunningstatus'), nullable=True),
    sa.Column('motor_type', sa.Enum('enrichment', 'eligibility', name='motortype'), nullable=True),
    sa.Column('operation_id', sa.Integer(), nullable=True),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['operation_id'], ['operations.id'], ),
    ...
    op.create_index(op.f('ix_motor_runnings_id'), 'motor_runnings', ['id'], unique=False)
    ...
    sa.Column('pipeline_status', sa.Enum('ENTRY', 'ENRICHMENT', 'ELIGIBILITY', 'CONTACT', 'WAITING_FILL', 'CONTRACT', 'GENERAL_VERIFICATION', 'WAITING_FOR_ANALYZES', 'IN_QUEUE', 'WAITING_PAYMENT', 'COMPLETED_WITH_SUCCESS', 'COMPLETED_WITH_FAILURE', 'PENDING_DOCS', name='clientoperationpipelinestatus'), nullable=True),
    ...
    sa.Column('search_irpf_status', sa.Enum('NOT_SEARCHED', 'SEARCHING', 'SUCCESS', 'ERROR', name='clientoperationsearchirpfstatus'), nullable=True),
    ...
    op.create_index(op.f('ix_client_operations_id'), 'client_operations', ['id'], unique=False)
    op.create_table('notifications',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('is_read', sa.Boolean(), nullable=True),
    sa.Column('text', sa.String(), nullable=True),
    sa.Column('type_alert', sa.Enum('waiting_register', 'error_in_consult_register', 'register_finished', 'task_to_do', name='notificationtypealert'), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('client_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['client_id'], ['clients.id'], ),
    ...
    sa.Column('type_task', sa.Enum('note', 'task', name='tasktypetask'), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('client_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['client_id'], ['clients.id'], ),
    ...
    sa.Column('pipeline_status', sa.Enum('entry', 'enrichment', 'eligibility', 'contact', 'waiting_fill', 'contract', 'general_verification', 'waiting_for_analyzes', 'in_queue', 'waiting_payment', 'completed_with_success', 'completed_with_failure', 'pending_docs', name='timelinepipelinestatus'), nullable=True),
    sa.Column('sended_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('source', sa.Enum('spreadsheet', 'landing_page', 'pending_registration', name='timelinesource'), nullable=True),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('client_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['client_id'], ['clients.id'], ),
